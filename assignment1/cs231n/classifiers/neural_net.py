...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # classifier loss.                                                          #
    #############################################################################
    h2 -= np.amax(h2, axis=1, keepdims=True)
    ...
    #############################################################################
    dprobs = probs
    dprobs[range(h2.shape[0]), y] -= 1
    dprobs = dprobs / num_train
    dW2 = h1.T.dot(dprobs)
    db2 = np.sum(dprobs, axis=0)
    dh1 = dprobs.dot(W2.T)
    dRelu = dh1
    dRelu[np.dot(X, W1) + b1 <= 0] = 0
    dW1 = X.T.dot(dRelu)
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      #########################################################################
      idxs = np.random.choice(X.shape[0], batch_size, replace=True)
      X_batch = X[idxs]
      y_batch = y[idxs]
      #########################################################################
      #                             END OF YOUR CODE                          #
  ...
  def predict(self, X):
    ...
    h2 = h1.dot(self.params['W2']) + self.params['b2']
    h2 -= np.amax(h2, axis=1, keepdims=True)
    probs = np.exp(h2) / np.sum(np.exp(h2), axis=1, keepdims=True)
